...
import { CATEGORY_KEY_TO_LABEL, CATEGORY_TREE, getSubcategoriesByKey, parseCategoriaLabel } from '~/constantes/categorias'
...
async function loadEditingItem() {
  ...
  if (!currentUser?.token) {
    ...
    await router.push('/autenticacion?mode=login&redirect=' + encodeURIComponent(`/vender?edit=${editingItemId.value}`))
    return
  }
...
async function onSubmit() {
  ...
  if (!currentUser?.token) {
    uiMessages.info('Inicia sesion para publicar tu articulo.')
    await router.push('/autenticacion?mode=login&redirect=/vender')
    return
  }
